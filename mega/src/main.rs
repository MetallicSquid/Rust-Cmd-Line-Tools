...
fn metadata_to_string(metadata: std::fs::Metadata, flags: std::collections::HashMap<&str, bool>) -> std::string::String {
    ...
    let mut created_meta = "Not supported on this platform, sorry.".to_string();
    ...
    if true_flags.is_empty() == false {
        for (key, _value) in true_flags {
            ...
            match key {
                ...
                "l" => {final_meta.push_str("File length: "); final_meta.push_str(&len_meta); final_meta.push_str(" bytes"); final_meta.push_str("\n");},
                ...
                _ => println!("This should never be triggered. Please inform MetallicSquid on github."),
            }
        }
    } else {
